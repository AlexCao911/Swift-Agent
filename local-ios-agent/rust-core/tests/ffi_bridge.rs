...
use local_ios_agent_runtime::ffi_bridge::{
    ...
    local_agent_runtime_bridge_new, local_agent_runtime_bridge_send_message,
...
fn bridge_exposes_session_turn_and_prompt_snapshot_json() {
    ...
    assert_eq!(session, json!("session_1"));
    assert_eq!(
    ...
    let turn = decode(
        &bridge
            .send_message_json(
                r#"{"session_id":"session_1","parent_event_id":null,"text":"hello"}"#,
            )
            .unwrap(),
    );

    assert_eq!(turn["run_id"], "run_3");
    assert_eq!(turn["state"], "completed");
    assert_eq!(turn["pending_tool_call_id"], Value::Null);
    assert_eq!(turn["events"][0]["kind"], "user_message");
    assert_eq!(turn["events"][0]["session_id"], "session_1");
...
fn bridge_registers_tool_schema_and_completes_tool_lifecycle() {
    let mut bridge = bridge();
    bridge.create_session_json().unwrap();
    bridge
    ...
    let turn = decode(
        &bridge
            .send_message_json(
                r#"{"session_id":"session_1","parent_event_id":null,"text":"use tool debug.echo"}"#,
            )
            .unwrap(),
    );
    ...
    assert_eq!(pending[0]["session_id"], "session_1");
...
fn bridge_exposes_and_resolves_approval_requests_json() {
    ...
    assert_eq!(approvals[0]["approval_id"], "approval_entry_6");
    assert_eq!(approvals[0]["requires_local_authentication"], true);

    let resumed = decode(
        &bridge
            .submit_approval_response_json(
                r#"{"approval_id":"approval_entry_6","approved":true,"reason":null}"#,
            )
            .unwrap(),
    );
