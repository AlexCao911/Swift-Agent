...
| Reliable host transport adapters | `rust-core/src/host_adapter`, existing `rust-core/src/execution/host_llm_worker.rs`, `LocalAgentLLMHost/ModelRuntimeCommandHandler.swift`, `ToolBatchCommandHandler.swift` | Rust runtime and `LocalAgentLLMHost` | LocalAgent source | Thin adapters connect the Rust `ModelRuntime`/`ToolRuntime` interfaces to the existing outbox, receipt, epoch and backpressure machinery; Swift executes exactly one accepted model command or tool batch and owns no Agent-loop state |
