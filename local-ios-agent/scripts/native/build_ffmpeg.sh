...
# .framework bundle in deps/platforms/<platform>/frameworks/. The umbrella
# FFmpeg.framework is a thin wrapper that re-exports them and includes
# fftools (ffmpeg_main).
#
...
#
# Usage:
#   OPENMINIS_PLATFORM=iphonesimulator ./build_ffmpeg.sh
#   ./build_ffmpeg.sh [clean]  # iphoneos by default
#
# Output:
#   deps/platforms/<platform>/frameworks/FFmpeg.framework/
#   deps/platforms/<platform>/frameworks/lib{avcodec,avformat,avutil,avfilter,swresample,swscale}.framework/
# ============================================================================

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$SCRIPT_DIR/native_platform.sh"
source "$SCRIPT_DIR/common.sh"
IFS='|' read -r FFMPEG_LOCKED_NAME FFMPEG_VERSION FFMPEG_URL FFMPEG_DIGEST \
    <<< "$(native_source_record ffmpeg "$LOCALAGENT_NATIVE_LOCK")"
FFMPEG_ARCHIVE="$LOCALAGENT_NATIVE_DOWNLOAD_ROOT/ffmpeg-${FFMPEG_VERSION}.tar.xz"
FFMPEG_DIR="$LOCALAGENT_NATIVE_SOURCE_ROOT/ffmpeg-${FFMPEG_VERSION}"
FFMPEG_PATCH_DIR="$LOCALAGENT_NATIVE_ROOT/Patches/FFmpeg"
BUILD_DIR="$OPENMINIS_NATIVE_BUILD_ROOT/ffmpeg"
FRAMEWORKS_BASE="$OPENMINIS_NATIVE_ARTIFACT_ROOT/frameworks"
FRAMEWORK_DIR="$FRAMEWORKS_BASE/FFmpeg.framework"
...
# ============================================================================
clean_build() {
    log_info "Cleaning $OPENMINIS_PLATFORM build artifacts..."
...
fetch_ffmpeg_source() {
    ...
    tmp="$(mktemp -d "$LOCALAGENT_NATIVE_SOURCE_ROOT/ffmpeg.XXXXXX")"

    log_info "FFmpeg source not present — preparing locked ${FFMPEG_VERSION} source..."
    download_locked_source ffmpeg "$LOCALAGENT_NATIVE_LOCK" "$FFMPEG_ARCHIVE"

    log_info "Extracting..."
    if ! tar -xf "$FFMPEG_ARCHIVE" -C "$tmp"; then
        rm -rf "$tmp"
        log_error "Failed to extract $tarball"
    fi

    mv "$tmp/ffmpeg-${FFMPEG_VERSION}" "$FFMPEG_DIR"
    rm -rf "$tmp"
    log_success "FFmpeg ${FFMPEG_VERSION} source ready at $FFMPEG_DIR"
}
...
apply_source_patches() {
    local patch_dir="$FFMPEG_PATCH_DIR"
...
# ============================================================================
configure_ffmpeg() {
    log_info "Configuring FFmpeg for $OPENMINIS_PLATFORM arm64..."
    ...
    if [ -f "$FFMPEG_DIR/config.h" ] || [ -f "$FFMPEG_DIR/ffbuild/config.mak" ]; then
        log_info "Cleaning legacy in-source FFmpeg build..."
    ...
    LAME_FLAGS=""
    if [ -f "$OPENMINIS_NATIVE_ARTIFACT_ROOT/lame/lib/libmp3lame.a" ]; then
        log_info "Found libmp3lame — enabling MP3 encoding"
        LAME_FLAGS="--enable-libmp3lame"
        LAME_CFLAGS="-I$OPENMINIS_NATIVE_ARTIFACT_ROOT/lame/include"
        LAME_LDFLAGS="-L$OPENMINIS_NATIVE_ARTIFACT_ROOT/lame/lib"
    else
    ...
    "$FFMPEG_DIR/configure" \
        ...
        --cc="$CC" \
        --sysroot="$IOS_SDK" \
        --extra-cflags="-arch arm64 $OPENMINIS_MIN_VERSION_FLAG -DCONFIG_FFMPEG_MAIN=1 $LAME_CFLAGS" \
        --extra-ldflags="-arch arm64 $OPENMINIS_MIN_VERSION_FLAG -isysroot $IOS_SDK $LAME_LDFLAGS" \
        --enable-pic \
        --enable-videotoolbox \
...
build_fftools() {
    ...
    cp "$FFMPEG_PATCH_DIR/fftools_stdio_redirect.h" "$FFMPEG_DIR/fftools/"
    cp "$FFMPEG_PATCH_DIR/fftools_stdio_redirect.c" "$FFMPEG_DIR/fftools/"
    ...
    FFTOOLS_CFLAGS="-arch arm64 $OPENMINIS_MIN_VERSION_FLAG -isysroot $IOS_SDK"
    FFTOOLS_CFLAGS="$FFTOOLS_CFLAGS -I$BUILD_DIR -I$FFMPEG_DIR -I$INSTALL_DIR/include"
    ...
    REDIRECT_CFLAGS="-arch arm64 $OPENMINIS_MIN_VERSION_FLAG -isysroot $IOS_SDK"
    REDIRECT_CFLAGS="$REDIRECT_CFLAGS -I$BUILD_DIR -I$FFMPEG_DIR -I$INSTALL_DIR/include -Oz -fPIC"
...
<dict>
    ...
    <array>
        <string>${OPENMINIS_SUPPORTED_PLATFORM}</string>
    </array>
</dict>
</plist>
PLISTEOF
