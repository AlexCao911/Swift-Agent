...
final class RootfsManager: @unchecked Sendable {
    ...
    /// [T-mcp-cli-help-and-readonly-ios] Guest path of the bundled localagent-mcp-cli
    /// Python lib. Its files are mounted read-only (see applyDefaultMountOverlay).
    private static let mcpCliLibDir = "/usr/local/lib/localagent-mcp-cli"
    private static let mcpCliLibPrefix = "/usr/local/lib/localagent-mcp-cli/"
    ...
    func installIfNeeded() throws {
        ...
        // Pre-create /var/localagent/ shared directory structure
        let localagentSubdirs = ["var/localagent/attachments", "var/localagent/offloads", "var/localagent/workspace", "var/localagent/skills", "var/localagent/shared", "var/localagent/mcp-servers"]
        for subdir in localagentSubdirs {
    ...
    func reset(keepUserData: Bool = false) throws -> URL? {
        ...
        if ISHKernel.shared.isBooted {
            ...
            NSLog("RootfsManager: rootfs reset while kernel booted; terminal disabled until app relaunch")
        }
    ...
    func applyDefaultMountOverlay() {
        ...
        NSLog("RootfsManager: applying default_mount")

        guard let overlayURL = Bundle.main.url(forResource: "default_mount", withExtension: nil) else {
            NSLog("RootfsManager: default_mount not found in bundle")
            return
        }
        ...
        while let fileURL = enumerator.nextObject() as? URL {
            ...
            do {
                ...
                // like /usr/local/bin/localagent-open are always executable.
                ...
                // [T-mcp-cli-help-and-readonly-ios] The bundled localagent-mcp-cli
                ...
                NSLog("RootfsManager: default_mount error %@: %@", relativePath, String(describing: error))
            }
        }

        // [T-mcp-cli-help-and-readonly-ios] The per-file loop above stamped the
        // localagent-mcp-cli lib FILES read-only (0o444), but ensureParentDirsInMetaDB
        ...
        NSLog(
            "RootfsManager: default_mount complete: %d files in %.1fms (copy %.1fms, metadata %.1fms, marker %.1fms)",
            fileCount,
            totalMs,
            copyTotalMs,
            metaDbTotalMs,
