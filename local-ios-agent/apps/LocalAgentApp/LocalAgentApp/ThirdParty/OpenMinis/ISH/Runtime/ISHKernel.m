...
@implementation ISHKernel {
    ...
    NSString *_dnsHostPath; // Host path to resolv.conf (Library/LocalAgent/dns/resolv.conf)
...
- (instancetype)init {
    ...
    if (self) {
        ...
        _commandQueue = dispatch_queue_create("com.localagentapp.ish.command", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
/// The host file lives in Library/LocalAgent/dns/resolv.conf and can be freely
/// updated by the app at any time — changes are instantly visible inside iSH.
- (void)mountDnsConfig {
    NSFileManager *fm = [NSFileManager defaultManager];

    // Determine host path: Library/LocalAgent/dns/resolv.conf
    NSString *library = [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) firstObject];
    NSString *dnsDir = [library stringByAppendingPathComponent:@"LocalAgent/dns"];
    ...
    // Bind mount: /etc/resolv.conf -> Library/LocalAgent/dns/resolv.conf
...
#define KERNEL_ENVP_APPEND(s) do { \
    ...
    if (envp_pos + _len < sizeof(envp_buf) - 256) { \
        ...
        KERNEL_ENVP_APPEND("BROWSER=/usr/local/bin/localagent-open");
...
- (void)handleCommandOutputInternal:(NSString *)output {
    ...
    // Common prompt patterns: "$ ", "# ", "root@localagent:", etc.
