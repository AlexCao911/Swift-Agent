...
final class BrowserUseManager: NSObject, ObservableObject {
    ...
    /// Downloads are saved into that session's /var/localagent/workspace/ directory.
    ...
    /// Shared scheme handler for `localagent://` URLs across all browser tabs
    /// and in-app previews that embed a WKWebView.
    static let sharedLocalAgentSchemeHandler = LocalAgentURLSchemeHandler()
    ...
    private static func makeWebView(profile: UserAgentProfile,
        ...
        config.setURLSchemeHandler(Self.sharedLocalAgentSchemeHandler, forURLScheme: "localagent")

        // Bridge JS window.print() to the native iOS print dialog. Injected on
        // every page (localagent:// and external http/https) at document start so a
    ...
    private func presentPrintDialog() {
        ...
        printInfo.jobName = pageTitle.isEmpty ? "LocalAgent" : pageTitle
    ...
    private func evaluateJavaScriptBounded(_ js: String, timeout: TimeInterval = 10) async throws -> Any? {
        ...
        let wrapped = try await withCheckedThrowingContinuation {
            (cont: CheckedContinuation<UnsafeJSValue, Error>) in
            box.cont = cont
            box.onFinish = { timer.cancel() }
            timer.resume()
            self.webView.evaluateJavaScript(js) { value, error in
                if let error { box.finish(.failure(error)) }
                else { box.finish(.success(UnsafeJSValue(value))) }
            }
        }
    ...
    private final class ContinuationBox: @unchecked Sendable {
        ...
        var cont: CheckedContinuation<UnsafeJSValue, Error>?
        ...
        var onFinish: (() -> Void)?
        func finish(_ result: Result<UnsafeJSValue, Error>) {
            lock.lock()
    ...
    private struct UnsafeJSValue: @unchecked Sendable {
        let value: Any?
    ...
    private func navigate(to urlString: String?) async throws -> BrowserActionResult {
        ...
        // Only allow HTTP(S) and localagent:// schemes for direct navigation
        let scheme = url.scheme?.lowercased() ?? ""
        if !["http", "https", "localagent"].contains(scheme) {
            return .error("Cannot navigate to \(scheme):// URLs. Only http://, https://, and localagent:// are supported.")
        }
    ...
    private func getCookies(keywords: [String]?, fuzzy: Bool) async -> BrowserActionResult {
        ...
        let envGuestPath = "/var/localagent/offloads/\(envFilename)"
...
extension BrowserUseManager: WKNavigationDelegate {
    ...
    func webView(
            ...
            Task { @MainActor [weak self] in
                ...
                if let cont = self.navigationContinuation {
                    ...
                    cont.resume(throwing: URLError(.unsupportedURL, userInfo: [
                        NSLocalizedDescriptionKey: "Blocked navigation to \(scheme):// — only http(s) and localagent:// are allowed in the browser."
                    ]))
                }
            }
            return
        ...
        let mode: WKWebpagePreferences.ContentMode = wantsDesktop ? .desktop : .mobile
...
extension BrowserUseManager: WKDownloadDelegate {
    nonisolated func download(
        ...
        Task { @MainActor in
            ...
            let dir = LocalAgentBrowserFiles.workspace(for: sid)
            try? FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)

            // Unique filename: name.ext, name-1.ext, name-2.ext, …
            // WKDownload requires that the destination file does not exist.
            var name = LocalAgentBrowserFiles.safeComponent(suggestedFilename)
    ...
    private func notifySessionChat(sid: String, text: String, icon: String) {
        // Rust is the sole transcript writer. Download status remains
        // product-local and is surfaced by BrowserDownloadCenter.
        logger.info("[BrowserDownload] session=\(sid) icon=\(icon) \(text)")
    }
...
// MARK: - LocalAgentURLSchemeHandler

/// Handles `localagent://` URLs in WKWebView by resolving them to local files.
/// Supports the four declared virtual host mounts.
final class LocalAgentURLSchemeHandler: NSObject, WKURLSchemeHandler {
    private let logger = AppLogger(category: "LocalAgentScheme")

    func webView(_ webView: WKWebView, start urlSchemeTask: any WKURLSchemeTask) {
        let url = urlSchemeTask.request.url!
        logger.info("[LocalAgentScheme] start \(url.absoluteString)")

        guard let fileURL = LocalAgentBrowserFiles.resolve(url) else {
            logger.warning("[LocalAgentScheme] not found: \(url.absoluteString)")
            urlSchemeTask.didFailWithError(URLError(.fileDoesNotExist))
            return
        }

        guard let data = try? Data(contentsOf: fileURL) else {
            logger.warning("[LocalAgentScheme] read failed: \(fileURL.path)")
        ...
        logger.info("[LocalAgentScheme] served \(url.absoluteString) → \(fileURL.lastPathComponent) (\(data.count) bytes, \(mimeType))")
    }
...
final class BrowserDownloadCenter: ObservableObject {
    ...
    func began(sessionId: String, filename: String, progress: Progress,
        ...
        queueAgentEvent(sessionId: sessionId, filename: filename,
                        line: "\(filename): download started → saving to \(LocalAgentBrowserFiles.downloadToolURL(sessionID: sessionId, filename: filename))")
        return id
    }
    ...
    func finished(id: UUID, sizeText: String) {
        ...
        queueAgentEvent(sessionId: e.sessionId, filename: e.filename,
                        line: "\(e.filename): download COMPLETED (\(sizeText)) → \(LocalAgentBrowserFiles.downloadToolURL(sessionID: e.sessionId, filename: e.filename)) (shell path: \(LocalAgentBrowserFiles.downloadLinuxPath(sessionID: e.sessionId, filename: e.filename)))")
    }
    ...
    func agentReport(for sessionId: String) -> String? {
        ...
        var lines: [String] = inflight.map { e in
            ...
            return "\(e.filename): downloading \(pct) (\(done) / \(total)) → saving to \(LocalAgentBrowserFiles.downloadToolURL(sessionID: e.sessionId, filename: e.filename))"
        }
        ...
        return "[browser_downloads] The browser is handling file download(s) NATIVELY "
            ...
            + "shell `ls -l /var/localagent/workspace/`) instead of downloading in parallel."
    }
