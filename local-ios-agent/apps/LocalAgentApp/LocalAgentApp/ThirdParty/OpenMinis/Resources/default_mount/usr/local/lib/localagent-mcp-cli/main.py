#!/usr/bin/env python3
"""localagent-mcp-cli — MCP client CLI for the LocalAgent agent (iSH / PRoot).
...
Diagnostics go to /var/localagent/mcp-servers/mcp-cli.log, never to stdout.
...
published in /tmp/localagent-mcp-daemon.port. info / add / remove / enable / disable
...
LOG_PATH = "/var/localagent/mcp-servers/mcp-cli.log"

PID_FILE = "/tmp/localagent-mcp-daemon.pid"
PORT_FILE = "/tmp/localagent-mcp-daemon.port"  # daemon publishes its 127.0.0.1 port here
LOCK_FILE = "/tmp/localagent-mcp-daemon.lock"  # cold-start fork guard (one winner forks)
...
def maybe_start_daemon():
    ...
    try:
        ...
        import logging
        daemon_log = "/var/localagent/mcp-servers/mcp-daemon.log"
        try:
...
def cmd_add(args, pretty):
    ...
    if wants_oauth:
        out["oauth_note"] = (
            ...
            "the CLI — open LocalAgent Settings > MCP Integrations > %s and tap "
...
USAGE = """localagent-mcp-cli — MCP (Model Context Protocol) client for the LocalAgent agent.

Usage: localagent-mcp-cli <command> [args] [--pretty]
...
Commands:
  ...
  add ... --oauth-client-id <id> --oauth-auth-endpoint <url> --oauth-token-endpoint <url>
      [--oauth-client-secret <secret>] [--oauth-scopes "s1 s2"] [--oauth-redirect-uri <uri>]
                                        ...
                                        LocalAgent Settings > MCP Integrations >
                                        <server> > Authorize.
...
Files:
  Servers:  /var/localagent/mcp-servers/servers.json   (mcpServers object, Claude-Desktop compatible)
  Log:      /var/localagent/mcp-servers/mcp-cli.log

Examples:
  localagent-mcp-cli list --pretty
  localagent-mcp-cli tools notion
  localagent-mcp-cli call notion search --input '{"q":"x"}'
  localagent-mcp-cli add --name notion --url https://mcp.notion.so/mcp --header "Authorization: Bearer $NOTION_TOKEN"
  localagent-mcp-cli add --name github --command npx --args "-y @modelcontextprotocol/server-github" --env "GITHUB_TOKEN=$GITHUB_TOKEN"
  localagent-mcp-cli add --name atlassian --command uvx --args "mcp-atlassian" --startup-timeout 120
  localagent-mcp-cli add --name gworkspace --url https://my-gws-mcp.example.com/mcp \
...
Server startup timeout (LocalAgent config, not part of the MCP protocol):
...
def main():
    ...
    try:
        if cmd == "list":
            ...
            sys.stderr.write("Run 'localagent-mcp-cli --help' for usage.\n")
